...
class NeuroAutoEncoder(NeuroBase):
    """Feed Forward Auto Encoder Neural Network whose weights are evolved.

        NeuroAutoEncoder evolves the weights of a static feed forward neural network
        ...
        layer_nodes : list of integers
                   A list containing the number of nodes for each hidden layer. The
                   number of layers should be odd, where the first half of the list
                   represents the layers for the encoder and the last half of the list
                   represents the layers for the decoder. For example, if
                   `layer_nodes` = `[25, 10, 3, 10, 25]` then the encoder has layers
                   `[input size, 25, 10, 3]` while the decoder has layers `[3, 10, 25, input size]`.
    ...
    def encode(self, data):
        """Encodes the data by compressing the data through the encoder using
         the best model.
    ...
    def decode(self, data):
        """Decodes the data by expanding the data through the decoder using
                the best model.
    ...
    def predict(self, data):
        """Predicts outcome of the data using the best model by running it through the
        encoder and then the decoder.
